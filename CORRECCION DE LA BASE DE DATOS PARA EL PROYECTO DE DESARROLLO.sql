...
create PROCEDURE PA_Supervisor
WITH ENCRYPTION
AS
BEGIN
    ...
    DECLARE @DiaSemana INT = DATEPART(WEEKDAY, GETDATE()); -- Obtiene el día de la semana
    DECLARE @BitDiaSemana INT = POWER(2, @DiaSemana - 1); -- Calcula el bit del día de la semana
    ...
    WHERE 
        ...
        AND (
            (@DiaSemana BETWEEN DATEPART(WEEKDAY, C.inicioDia) AND DATEPART(WEEKDAY, C.finDia))
            OR
...
CREATE PROCEDURE PA_Supervisor_Excel
AS
BEGIN
    -- Seleccionar los datos base necesarios para la exportación
    SELECT 
        Docente,
        Asignatura,
        Seccion,
        ...
        Fecha,
        CASE 
            WHEN Asistencia = 1 THEN 'P' -- Presente
            ELSE 'A' -- Ausente
        END AS EstadoAsistencia
    FROM AsistenciaSupervisor
    ORDER BY Docente, Fecha;
END;
...
alter PROCEDURE PA_Justifica
    @CodigoDecano int
AS
BEGIN
    SET NOCOUNT ON;

    -- Validar que el parámetro @CodigoDecano no sea nulo o vacío
    IF @CodigoDecano IS NULL OR LEN(@CodigoDecano) = 0
    BEGIN
        PRINT 'El parámetro @CodigoDecano está vacío o es nulo.';
        RETURN;
    END
    ...
    -- Validar si no se encontró un código de facultad
    IF @codigo_facu IS NULL
    BEGIN
        PRINT 'No se encontró un código de facultad para el decano especificado.';
        RETURN;
    END
